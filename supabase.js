...
async function guardarFeedback(comentario) {
    try {
        const { error } = await supabaseClient
            ...
            .insert({ comentario, user_agent: navigator.userAgent, url_pagina: window.location.href });
...
async function registrarVisita(pagina = "inicio") {
    try {
        const { error } = await supabaseClient
            .from("visitas_pagina")
            .insert({ pagina, referrer: document.referrer || "directo", user_agent: navigator.userAgent });
